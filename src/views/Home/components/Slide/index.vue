<template>
	<div class="relative mx-auto w-11/12 slide">
		<vueper-slides
			autoplay
			:bullets="false"
			:visible-slides="3"
			...
			<vueper-slide v-for="item in banners" :key="item.bannerId" :image="item.pic"/>
		</vueper-slides>
	</div>
</template>
...
.vueperslides__paused{top:.7em;right:.7em;opacity:0;text-shadow:0 0 3px rgba(0,0,0,.4);z-index:1}.vueperslides:hover .vueperslides__paused{opacity:1}
...
.vueperslides__bullet .default{width:12px;height:12px;border-radius:12px;border:1px solid currentColor;background-color:transparent;box-shadow:0 0 1px rgba(0,0,0,.5),0 0 3px rgba(0,0,0,.3);transition:.4s ease-in-out;box-sizing:border-box}
...
.vueperslides{position:relative}.vueperslides--fixed-height .vueperslide,.vueperslides--fixed-height .vueperslides__inner,.vueperslides--fixed-height .vueperslides__parallax-wrapper{height:inherit}
...
.vueperslides__arrow{position:absolute;font-size:inherit;color:inherit;text-align:center;transform:translateY(-50%);cursor:pointer;user-select:none;outline:none;z-index:2;line-height:1}
...
.vueperslides__bullet,.vueperslides__bullets button{cursor:pointer;user-select:none;outline:none;z-index:2;display:flex;justify-content:center;align-items:center;color:inherit}
