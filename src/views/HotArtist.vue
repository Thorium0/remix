  ...
  <div class="hot-artists">
    <div class="hot-artists-area">
      ...
      <span
        ...
        @click="_setParams({ type: 'area', value: item.value })"
        >{{ item.label }}</span
      >
    </div>
    <div class="hot-artists-type">
      ...
      <span
        ...
        @click="_setParams({ type: 'type', value: item.value })"
        >{{ item.label }}</span
      >
    </div>
    <div class="hot-artists-initial">
      筛选：
      <span
        :class="{ active: params.initial === '-1' }"
        @click="_setParams({ type: 'initial', value: '-1' })"
        >热门</span
      >
      <span
        v-for="(item, key) in initial"
        :key="key"
        :class="{ active: params.initial === item }"
        @click="_setParams({ type: 'initial', value: item })"
        >{{ item }}</span
      >
    </div>
